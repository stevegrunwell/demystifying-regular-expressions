...
footer.innerHTML = '<a href="https://phpc.social/@stevegrunwell">phpc.social/@stevegrunwell</a> - <a href="https://stevegrunwell.com/slides/regex">stevegrunwell.com/slides/regex</a>';
...
// Register a custom <regex-demo> web component.
class RegexDemo extends HTMLElement {
    constructor() {
        super();
    }

    connectedCallback() {
        let samp = this.querySelector('samp');
        let output = this.querySelector('output');

        // We have all the necessary markup, so don't do anything.
        if (samp && output) {
            return;
        }
        ...
        if (!samp) {
            ...
            samp.innerText = this.getAttribute('pattern');
        }
        ...
        this.replaceChildren(samp, output);
    }
}
customElements.define('regex-demo', RegexDemo);
